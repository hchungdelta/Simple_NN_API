...
class attention:
    ...
    def forward(self, encoder_hidden_layer, decoder_hidden_layer):
        ...
        if self.attention_type == 'general':
            self.this_context_vector = np.zeros((self.decoder_timestep, self.batch, self.attention_hidden_units))
            ...
            for this_decoder_timestep in range(self.decoder_timestep):
                score = np.einsum('Ebd,bd->bE', encoder_hidden_layer, decoder_hidden_layer[this_decoder_timestep])
                for this_batch in range(self.batch):
                    this_alpha[this_decoder_timestep, this_batch] = np.exp(score[this_batch, :])/np.sum(np.exp(score[this_batch, :]))
            self.this_context_vector = np.einsum('DbE,Ebd->Dbd', this_alpha, encoder_hidden_layer)
    ...
    #@jit(fastmath=True)
    def backprop(self, dLoss):
        self.alpha = np.array(self.alpha).reshape(self.total_decoder_timestep, self.batch, self.encoder_timestep)
        self.all_decoder_hidden_layer = np.array(self.all_decoder_hidden_layer).reshape(self.total_decoder_timestep, self.batch, self.depth)
        "You are doing att only mode!!!  / no W"
        #dc = dLoss
        dc = dLoss#[:,:,1*self.attention_hidden_units:]
        self.dW = np.zeros_like(self.W)

        correlate_alpha = np.einsum('DbE,Dbe->DbEe', -1*self.alpha, self.alpha)
        correlate_enco = np.einsum('Ebd,ebd->Eebd', self.encoder_hidden_layer, self.encoder_hidden_layer)
        ddeinp_first_term = np.einsum('DbE,Ebd->DEbd', self.alpha, self.encoder_hidden_layer**2)
        ddeinp_second_term = np.einsum('DbEe,Eebd->DEbd', correlate_alpha, correlate_enco)
        self.ddeInp = np.einsum('Dbd,DEbd->Dbd', dc, ddeinp_first_term + ddeinp_second_term)

        correlate_deco = np.einsum('Dbd,Ebd->DEbd', dc*self.all_decoder_hidden_layer, self.encoder_hidden_layer)
        #correlate_alpha  = np.einsum('DbE,Dbe->DbEe',-1*self.alpha,self.alpha )
        self.dInp = np.einsum('DbE,Dbd->Ebd', self.alpha, dc)
        self.dInp += np.einsum('DbE,DEbd->Ebd', self.alpha, correlate_deco)
        self.dInp += np.einsum('DbEe,DEbd->Ebd', correlate_alpha, correlate_deco)
