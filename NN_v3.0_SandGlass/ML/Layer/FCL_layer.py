...
class Full_Connected_Layer():
    def __init__(self, hidden_unit, random=False):
...
class Full_Connected_Layer_NoBias():
    def __init__(self, hidden_unit):
        # hidden_unit : [input_hidden_units, output_hidden_units]
        self.W = np.random.normal(size=[hidden_unit[0], hidden_unit[1]])*0.01
        self.sum_dW = np.zeros_like(self.W)
        self.timepiece = []
        self.output = None
    def update(self, dW, db, lr):
...
class Embedding():
    ...
    def __init__(self, word2vector_array, trainable=False, random=False):
    ...
    def select(self, x):
        ...
        self.batch = x.shape[1]
        this_output = np.zeros((self.timestep, self.batch, self.vector_length))
...
class BatchNorm(Full_Connected_Layer):
    def __init__(self):
        self.W = 1-np.random.random(1)*0.1  # rquals to gamma
    ...
    def forward(self, inp):
        ...
        self.norm_x = self.diff/np.sqrt(self.mb_var + self.eps)
    ...
    def backprop(self, dLoss):
        ...
        d_mb_var = -0.5*d_norm_x*self.diff*((self.mb_var +self.eps)**(-3./2))
        d_mb_mean = (-1*d_norm_x/np.sqrt(self.mb_var +self.eps))+d_mb_var*(-2)*self.normalization
        self.sum_db = self.db
        self.sum_dW = self.dW
        dL_prev = (d_norm_x/np.sqrt(self.mb_var + self.eps)) +\
            d_mb_var*self.diff*2*self.normalization+ d_mb_mean*self.normalization
        return dL_prev
