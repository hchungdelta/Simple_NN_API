...
class CNN_layer(activation):
    def __init__(self, inp_layer_size, stride=(1, 1), kernel_size=(2, 2), paddling=True, activation="None"):
        ...
        if self.paddling:
            ...
            if self.stride_x > 1:
                outx_size = 1+int(np.ceil((self.inpx_size-self.kernel_x)/self.stride_x))
        else :
            outy_size = 1+int((self.inpy_size-self.kernel_y)/self.stride_y)
    ...
    def update(self, dW, db, lr):  
        ...
        normalized_factor = 1+abs(self.SUM_inp)*self.kernel_y*self.kernel_x*self.amount_of_input_filters   #*self.amount_of_output_filters

        limitb = np.max(abs(db))
        limitw = np.max(abs(dW))
        limit = np.max([limitb,limitw])/normalized_factor
        while limit < 0.01:
                limit = limit*2
                db = db*2
                dW = dW*2
        ...
        self.bias= self.bias - lr*(1/normalized_factor)*db
        self.kernel = self.kernel -lr*(1/normalized_factor)*dW
