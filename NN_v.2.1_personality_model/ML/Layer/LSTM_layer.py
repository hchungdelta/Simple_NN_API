...
class LSTMcell:
    ...
    """
    def __init__(self, input_info, output_form="All", ortho=False,
                 LSTM_LN=False, smooth=1, dtype=np.float32):
        '''
...
class persona_concat:
    def __init__(self):
        self.inp1_depth = 0
    def forward(self, inp1, inp2):
        self.inp1_depth = inp1.shape[2]
        return np.concatenate((inp1,inp2),axis=2)
    def timestep_forward(self, inp1, inp2):
        self.inp1_depth = inp1.shape[1]
        return np.concatenate((inp1,inp2),axis=1)
    def backprop(self, dLoss):
        return dLoss[:, :, :self.inp1_depth], dLoss[:, :, self.inp1_depth:]
