...
from ML.Layer.lstmhelper import Sigmoid, Tanh, ThreeD_onehot, timestep_square_loss, timestep_softmax_cross_entropy, timestep_softmax
from ML.Layer.Normalization import LSTM_BatchNorm
class LSTMcell:
    """
    ...
    Hence it can easily and efficiently performes forward & backward propagation.
    """
    def __init__(self, input_info, output_form="All", ortho=False, LSTM_BN=False, smooth=1, dtype=np.float32):
        '''
        ...
        LSTM_BN:  whether to switch on inner batch normalization.
        '''
        ...
        self.share_mem = [self.batch, self.input_depth, self.hidden_units,
                          self.total_depth, self.W, self.b, self.smooth, dtype, LSTM_BN]
    ...
    def forward(self, inputdata, prev_h_state, prev_c_state, cutoff_length=None):
        ...
        if not isinstance(inputdata, np.ndarray):
            inputdata = np.zeros((self.length, self.batch, self.input_depth)).astype(dtype)
        prev_h_list = []
        prev_c_list = []
    ...
    def timestep_forward(self, inputdata, prev_h_state, prev_c_state, cutoff_length=None):
        ...
        if not isinstance(inputdata, np.ndarray):
            inputdata = np.zeros((self.batch, self.input_depth)).astype(dtype)
        #prev_h_list = []
        #prev_c_list = []
...
class LSTM():
    def __init__(self, share_mem):
        ...
        self.LSTM_BN_on = share_mem[8]
        if  self.LSTM_BN_on:
            self.LSTM_BN_inp = LSTM_BatchNorm((4*self.hidden_units))
            self.LSTM_BN_hid = LSTM_BatchNorm((4*self.hidden_units))
            self.LSTM_BN_state = LSTM_BatchNorm((self.hidden_units))
    ...
    def forward(self, inp_layer, prev_state_h, prev_state_c):
        ...
        if self.LSTM_BN_on:
            inp_part = np.zeros((self.batch, 4*self.hidden_units)).astype(self.dtype)
            hid_part = np.zeros((self.batch, 4*self.hidden_units)).astype(self.dtype)
            xW = np.zeros((self.batch, 4*self.hidden_units)).astype(self.dtype)
            for inner_idx in range(4):
                this_gate_W = self.W[inner_idx*self.total_depth:(inner_idx+1)*self.total_depth]
                W_for_inp = this_gate_W[self.hidden_units:]
                W_for_hid = this_gate_W[:self.hidden_units]
                lower_end = inner_idx*self.hidden_units
                upper_end = (inner_idx+1)*self.hidden_units
                inp_part[:, lower_end:upper_end] = np.matmul(self.inp_layer, W_for_inp)
                hid_part[:, lower_end:upper_end] = np.matmul(self.prev_h, W_for_hid)
            inp_part_BN = self.LSTM_BN_inp.forward(inp_part)
            hid_part_BN = self.LSTM_BN_hid.forward(hid_part)
            xW = inp_part_BN+hid_part_BN
            self.state = np.concatenate([self.prev_h, self.inp_layer], axis=1)
        if not self.LSTM_BN_on:
            self.state = np.concatenate([self.prev_h, self.inp_layer], axis=1)
            xW = np.zeros((self.batch, 4*self.hidden_units)).astype(self.dtype)
            for inner_idx in range(4):
    ...
    def backprop(self, dLoss, next_dh, next_dc):
        ...
        if isinstance(dLoss, np.ndarray):
            dL = (dLoss+next_dh)
            dc = dL*self.Ho*dTanh_o + next_dc
            dbo = dL*dSigmoid_Ho*self.Tanh_o     #output gate
        else:
            dc = next_dc + next_dh*self.Ho*dTanh_o
            dbo = next_dh*dSigmoid_Ho*self.Tanh_o    #output gate
        ...
        if self.LSTM_BN_on:
            self.local_dL = np.zeros((self.batch, self.total_depth)).astype(self.dtype)
            dBN_hid = self.LSTM_BN_hid.backprop(db)
            dBN_inp = self.LSTM_BN_inp.backprop(db)
            dBN = dBN_hid + dBN_inp
            #print("bef",np.mean(abs(db)),"aft",np.mean(abs(dBN)))
            for inner_idx in range(4):
                lower_end = inner_idx*self.total_depth
                mid_cut = inner_idx*self.total_depth+self.hidden_units
                upper_end = (inner_idx+1)*self.total_depth
                dBN_lower_end =  inner_idx*self.hidden_units
                dBN_upper_end = (1+inner_idx)*self.hidden_units
                self.local_dW[lower_end:mid_cut] = np.dot(self.prev_h.T,
                                                          dBN_hid[:, dBN_lower_end:dBN_upper_end])
                self.local_dW[mid_cut:upper_end] = np.dot(self.inp_layer.T,
                                                          dBN_inp[:, dBN_lower_end:dBN_upper_end])

                self.local_dL[:, :self.hidden_units] += np.matmul(dBN_hid[:, dBN_lower_end:dBN_upper_end], self.W[lower_end:mid_cut].T)
                self.local_dL[:, self.hidden_units:] += np.matmul(dBN_inp[:, dBN_lower_end:dBN_upper_end], self.W[mid_cut:upper_end].T)
