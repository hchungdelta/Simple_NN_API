...
class Model:
    ...
    def Forward(self, encoder_output_data, decoder_output_data, target,
                cutoff_length, show=False, show_type="all"):
        """
        ...
        if self.mode == 'infer':
            ...
            for timestep in range(cutoff_length[1]):
                ...
                for idx, func in enumerate(self.decoder_forward):
                    ...
                    if not self.decoder_isLSTM[idx] and not self.decoder_isCost[idx]:
        ...
        return (prediction, Loss) if not show else (prediction, output_dict)
    ...
    def Backprop(self, show=False, show_type="all"):
        ...
        for idx, func in enumerate(reversed(self.decoder_backprop)):
            ...
            if self.decoder_isCost[rev_idx]:
                dLoss = func()
            if not self.decoder_isLSTM[rev_idx] and not self.decoder_isCost[rev_idx]:
                dLoss = func(dLoss)
