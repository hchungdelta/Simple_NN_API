...
def swapper(array, swap_amount):
    """
    exchange the order of random two words in the input.
    swap_amount: the amount of pairs to be swapped
    """
    len_array = len(array)
...
def decode(input_list, reversed_dicts):
    ...
    for idx in input_list:
        try:
            ...
            if add_word != "<PAD>" and add_word != "unknown":
...
def input_training_data(encode_input_file, decode_target_file):
    with open(encode_input_file, 'r') as jsonfile:
        training_data = json.load(jsonfile)
        inputdata = training_data["inputs"]

    with open(decode_target_file, 'r') as jsonfile:
        training_data = json.load(jsonfile)
        targetdata = training_data["inputs"]
    return inputdata, targetdata

def input_Embedding(embed_file_name, dtype=np.float32):
    with open(embed_file_name, "r", encoding='utf-8') as jsonfile:
        Embedding = json.load(jsonfile)["embedding_matrix"]  #huge file...
    _embeddings = [float(i) for vector in Embedding  for i in vector]
    embeddings = np.reshape(_embeddings, [-1, 300]).astype(dtype)
    return  embeddings
