...
def swapper(array, swap_amount):
    """
    randomly swap two inputs, e.g.  I am here -> here am I
    """
    len_array = len(array)
...
def batch_helper(inputdata, targetdata, batch, lengthlimit, mode="cut"):
    ...
    for this_idx in range(batch):
        ...
        batch_decode_input[this_idx].insert(0, 0)
...
def input_Embedding(embed_file_name, dtype=np.float32):
    with open(embed_file_name, "r", encoding='utf-8') as jsonfile:
        Embedding = json.load(jsonfile)["embedding"]  #huge file...
    _embeddings = [float(i) for vector in Embedding  for i in vector]
    embeddings = np.reshape(_embeddings, [-1, 200]).astype(dtype)
    return  embeddings
