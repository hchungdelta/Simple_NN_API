"""
Title : SandGlass add attention mechanism - reverse test
Description :
...
# encoder 2nd layer (multiple conv)
...
# Batch Normalization
BN0 = ML.Layer.Normalization.BatchNorm_4d((40), eps=1e-6, mode='infer')
BN1 = ML.Layer.Normalization.BatchNorm_4d((40), eps=1e-6, mode='infer')
BN2 = ML.Layer.Normalization.BatchNorm_4d((40), eps=1e-6, mode='infer')
BN3 = ML.Layer.Normalization.BatchNorm_4d((40), eps=1e-6, mode='infer')
BN4 = ML.Layer.Normalization.BatchNorm_4d((80), eps=1e-6, mode='infer')
BN5 = ML.Layer.Normalization.BatchNorm_4d((80), eps=1e-6, mode='infer')
BN6 = ML.Layer.Normalization.BatchNorm_4d((80), eps=1e-6, mode='infer')
BN7 = ML.Layer.Normalization.BatchNorm_4d((80), eps=1e-6, mode='infer')
# others
...
for step in range(1):
    ...
    pred, _, _ = ML.NN.Loss.timestep_softmax_cross_entropy(output, target)

    if rank == 0 and step%display == 0:
        print("alpha 0", [attn0.get_alpha()[0, :, 0, :]])
        print("alpha 1a", [attn1.get_alpha()[0, :, 0, :]])
        print("alpha 1b", [attn1.get_alpha()[1, :, 0, :]])
        print("alpha 2", [attn2.get_alpha()[0, :, 0, :]])
        print("alpha 3", [attn3.get_alpha()[0, :, 0, :]])

        TARGET = np.argmax(target, axis=2).transpose(1, 0)
        PRED = np.argmax(pred, axis=2).transpose(1, 0)
        print("TARGET:", [TARGET[0]])
        print("PRED:", [PRED[0]])
        print("INP", [TARGET[0][::-1]])
